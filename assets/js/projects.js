...
async function displayProjects() {
    ...
    const projectCards = mainContainer.selectAll('div.project-card')
        .data(projects)
        .enter()
        .append('div')
        .attr('class', d => `project-card${d.featured ? ' featured' : ''}${d.mlops ? ' mlops' : ''}`);

    // Add project images
    const imageDiv = projectCards.append('div')
    ...
    // Add project details
    const detailsDiv = projectCards.append('div')
        .attr('class', 'project-details col-sm-9');

    detailsDiv.append('h5')
        .append('a')
    ...
    const descriptionDiv = detailsDiv.append('div')
        .attr('class', 'project-description')
        .html(d => d.description.join('<br>'));

    // Add project links
    const linksDiv = detailsDiv.append('div')
